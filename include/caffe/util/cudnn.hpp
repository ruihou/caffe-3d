...
template <typename Dtype>
inline void setTensorNdDesc(cudnnTensorDescriptor_t* desc, std::vector<int> shape) {
  // Set up stride.
...
inline void setNdConvolutionDesc(cudnnConvolutionDescriptor_t* conv,
  ...
  CUDNN_CHECK(cudnnSetConvolutionNdDescriptor(*conv, pad.size(), pad.data(),
      stride.data(), upscale.data(), CUDNN_CROSS_CORRELATION, cudnn_type));
}
